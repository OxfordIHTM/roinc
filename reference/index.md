# Package index

## Setup API credentials

- [`roinc_set_credentials()`](https://oxford-ihtm.io/roinc/reference/roinc_set_credentials.md)
  : Set LOINC API credentials

## Terminology Service

- [`roinc_codesystem()`](https://oxford-ihtm.io/roinc/reference/roinc_codesystem.md)
  [`roinc_codesystem_lookup()`](https://oxford-ihtm.io/roinc/reference/roinc_codesystem.md)
  : Retrieving the LOINC CodeSystem resource for FHIR
- [`roinc_valueset()`](https://oxford-ihtm.io/roinc/reference/roinc_valueset.md)
  [`roinc_valueset_validate()`](https://oxford-ihtm.io/roinc/reference/roinc_valueset.md)
  : Retrieving the LOINC value set resource for FHIR
- [`roinc_concept_map()`](https://oxford-ihtm.io/roinc/reference/roinc_concept_map.md)
  : Retrieving the LOINC concept map resource for FHIR
- [`roinc_questionnaire()`](https://oxford-ihtm.io/roinc/reference/roinc_questionnaire.md)
  : Retrieve LOINC panel/questionnaire resource for FHIR

## Search

- [`roinc_search_loincs()`](https://oxford-ihtm.io/roinc/reference/roinc_search.md)
  [`roinc_search_answerlists()`](https://oxford-ihtm.io/roinc/reference/roinc_search.md)
  [`roinc_search_parts()`](https://oxford-ihtm.io/roinc/reference/roinc_search.md)
  [`roinc_search_groups()`](https://oxford-ihtm.io/roinc/reference/roinc_search.md)
  : Search the LOINC database

## Metadata

- [`roinc_get_metadata()`](https://oxford-ihtm.io/roinc/reference/roinc_get_metadata.md)
  : Get LOINC's primary release artifact download metadata

## Download

- [`roinc_download()`](https://oxford-ihtm.io/roinc/reference/roinc_download.md)
  : Download LOINC's primary release artifact

## Data

- [`linguistic_variants`](https://oxford-ihtm.io/roinc/reference/linguistic_variants.md)
  : List of the LOINC Linguistic Variants that are available and their
  producers (translators).
